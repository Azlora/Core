...
""" Example logging Azlora Core mempool events using the mempool:added,
...
def main(azlorad_path):
    azlorad_with_usdts = USDT(path=str(azlorad_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    azlorad_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    azlorad_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    azlorad_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    azlorad_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[azlorad_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/azlorad")
        exit(1)
