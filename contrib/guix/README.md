# Bootstrappable Azlora Core Builds

This directory contains the files necessary to perform bootstrappable Azlora
Core builds.
...
To build Azlora Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Azlora Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/azlora-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/azlora-core/guix.sigs).
