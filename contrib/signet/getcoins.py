...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing azlora-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='azlora-cli', help='azlora-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Azlora address to which the faucet should send')
...
parser.add_argument('azlora_cli_args', nargs='*', help='Arguments to pass on to azlora-cli (default: -signet)')

args = parser.parse_args()

if args.azlora_cli_args == []:
    args.azlora_cli_args = ['-signet']


def azlora_cli(rpc_command_and_params):
    argv = [args.cmd] + args.azlora_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = azlora_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = azlora_cli(['getnewaddress', 'faucet', 'bech32'])
