...
"""Script for verifying Azlora Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from azloracore.org and azlora.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/azlora-core/guix.sigs/tree/main/builder-keys
...
HOST2 = "https://azlora.org"
VERSIONPREFIX = "azlora-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Azlora community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"azlora_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by azloracore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the azlora release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes azlora.org lags behind azloracore.org.)')
    )
