...
const char * const AZLORA_CONF_FILENAME = "azlora.conf";
const char * const AZLORA_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //azlora-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", AZLORA_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Azlora
    // macOS: ~/Library/Application Support/Azlora
    // Unix-like: ~/.azlora
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Azlora";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Azlora";
#else
    // Unix-like
    return pathRet / ".azlora";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", AZLORA_CONF_FILENAME));
}
