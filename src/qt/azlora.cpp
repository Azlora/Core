...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<AzloraUnit>("AzloraUnit");
#else
    qRegisterMetaType<AzloraUnit>("AzloraUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. azlora_de.qm (shortcut "de" needs to be defined in azlora.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. azlora_de_DE.qm (shortcut "de_DE" needs to be defined in azlora.qrc)
...
static const char* qt_argv = "azlora-qt";

AzloraApplication::AzloraApplication()
...
void AzloraApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the AzloraApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", AzloraGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void AzloraApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool AzloraApplication::createOptionsModel(bool resetSettings)
{
...
void AzloraApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new AzloraGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &AzloraGUI::quitRequested, this, &AzloraApplication::requestShutdown);
...
void AzloraApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void AzloraApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool AzloraApplication::baseInitialize()
{
    return node().baseInitialize();
}

void AzloraApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &AzloraApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &AzloraApplication::handleRunawayException);
    connect(this, &AzloraApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &AzloraApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void AzloraApplication::parameterSetup()
{
...
void AzloraApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void AzloraApplication::requestInitialize()
{
...
void AzloraApplication::requestShutdown()
{
...
void AzloraApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // azlora: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &AzloraGUI::handlePaymentRequest);
            connect(window, &AzloraGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void AzloraApplication::handleRunawayException(const QString &message)
{
...
void AzloraApplication::handleNonFatalException(const QString& message)
{
...
WId AzloraApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool AzloraApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AzloraGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(azlora);
    Q_INIT_RESOURCE(azlora_locale);
    ...
    /// 6-7. Parse azlora.conf, determine network, switch to network specific
    ...
    // azlora: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
