...
#include <qt/azloraaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Azlora address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new AzloraAddressEntryValidator(parent));
    widget->setCheckValidator(new AzloraAddressCheckValidator(parent));
}
...
bool parseAzloraURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no azlora: URI
    if(!uri.isValid() || uri.scheme() != QString("azlora"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!AzloraUnits::parse(AzloraUnit::AZL, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseAzloraURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseAzloraURI(uriInstance, out);
}

QString formatAzloraURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("azlora:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AzloraUnits::format(AzloraUnit::AZL, info.amount, false, AzloraUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openAzloraConf()
{
    ...
    /* Open azlora.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Azlora.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Azlora (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Azlora (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Azlora*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "azlora.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("azlora-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a azlora.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Azlora\n";
        else
            optionFile << strprintf("Name=Azlora (%s)\n", chain);
