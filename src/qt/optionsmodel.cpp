...
/** Call node.updateRwSetting() with Azlora 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Azlora 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/azlora/azlora/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because azlora 22.x will not try to read these.
...
//! Convert enabled/size values to azlora -prune setting.
...
//! Get pruning enabled value to show in GUI from azlora -prune setting.
...
//! Get pruning size value to show in GUI from azlora -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayAzloraUnit")) {
        settings.setValue("DisplayAzloraUnit", QVariant::fromValue(AzloraUnit::AZL));
    }
    QVariant unit = settings.value("DisplayAzloraUnit");
    if (unit.canConvert<AzloraUnit>()) {
        m_display_azlora_unit = unit.value<AzloraUnit>();
    } else {
        m_display_azlora_unit = AzloraUnit::AZL;
        settings.setValue("DisplayAzloraUnit", QVariant::fromValue(m_display_azlora_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if azlora.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_azlora_unit);
...
{
    if (new_unit.isNull() || new_unit.value<AzloraUnit>() == m_display_azlora_unit) return;
    m_display_azlora_unit = new_unit.value<AzloraUnit>();
    QSettings settings;
    settings.setValue("DisplayAzloraUnit", QVariant::fromValue(m_display_azlora_unit));
    Q_EMIT displayUnitChanged(m_display_azlora_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/azlora/azlora/pull/8273
    ...
    // (https://github.com/azlora-core/gui/issues/567).
