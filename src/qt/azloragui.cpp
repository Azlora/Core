...
const std::string AzloraGUI::DEFAULT_UIPLATFORM =
...
AzloraGUI::AzloraGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &AzloraGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &AzloraGUI::showModalOverlay);
...
void AzloraGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Azlora address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and azlora: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &AzloraGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &AzloraGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &AzloraGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Azlora addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Azlora addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Azlora Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Azlora Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a azlora: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Azlora command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &AzloraGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &AzloraGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &AzloraGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &AzloraGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &AzloraGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &AzloraGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &AzloraGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &AzloraGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &AzloraGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &AzloraGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &AzloraGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &AzloraGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &AzloraGUI::showDebugWindow);
}

void AzloraGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void AzloraGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &AzloraGUI::setCurrentWalletBySelectorIndex);
...
void AzloraGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &AzloraGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &AzloraGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &AzloraGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &AzloraGUI::showProgress);
...
#ifdef ENABLE_WALLET
void AzloraGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void AzloraGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &AzloraGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &AzloraGUI::removeWallet);
...
WalletController* AzloraGUI::getWalletController()
{
    return m_wallet_controller;
}

void AzloraGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &AzloraGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &AzloraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &AzloraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &AzloraGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &AzloraGUI::incomingTransaction);
    connect(this, &AzloraGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void AzloraGUI::removeWallet(WalletModel* walletModel)
{
...
void AzloraGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void AzloraGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void AzloraGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AzloraGUI::setWalletActionsEnabled(bool enabled)
{
...
void AzloraGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &AzloraGUI::toggleHidden);
...
void AzloraGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void AzloraGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AzloraGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void AzloraGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void AzloraGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void AzloraGUI::openClicked()
{
...
void AzloraGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AzloraGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AzloraGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AzloraGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AzloraGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AzloraGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void AzloraGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void AzloraGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Azlora network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void AzloraGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AzloraGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void AzloraGUI::updateHeadersSyncProgressLabel()
{
...
void AzloraGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void AzloraGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &AzloraGUI::quitRequested);
...
void AzloraGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void AzloraGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void AzloraGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &AzloraGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &AzloraGUI::show);
                e->ignore();
            }
        }
    }
...
void AzloraGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AzloraGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AzloraGUI::incomingTransaction(const QString& date, AzloraUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(AzloraUnits::formatWithUnit(unit, amount, true));
...
void AzloraGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AzloraGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AzloraGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AzloraGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AzloraGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void AzloraGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AzloraGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void AzloraGUI::updateProxyIcon()
{
    std::string ip_port;
...
void AzloraGUI::updateWindowTitle()
{
...
void AzloraGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AzloraGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AzloraGUI::detectShutdown()
{
...
void AzloraGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void AzloraGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AzloraGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = AzloraGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void AzloraGUI::subscribeToCoreSignals()
{
...
void AzloraGUI::unsubscribeFromCoreSignals()
{
...
bool AzloraGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<AzloraUnit> units = AzloraUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AzloraUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, AzloraUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const AzloraUnit u : AzloraUnits::availableUnits()) {
        menu->addAction(AzloraUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(AzloraUnit newUnits)
{
    setText(AzloraUnits::longName(newUnits));
}
