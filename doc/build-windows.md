...
Below are some notes on how to build Azlora Core for Windows.

The options known to work for building Azlora Core on Windows are:
...
Note that for WSL the Azlora Core source path MUST be somewhere in the default mount file system, for
example /usr/src/azlora, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\azlora`, for example:

    make install DESTDIR=/mnt/c/workspace/azlora
