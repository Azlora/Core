...
The Azlora Core daemon can be configured to act as a trusted "border
router", implementing the azlora wire protocol and relay, making
...
The ZeroMQ feature in Azlora Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building azlorad:
...
For instance:
    ...
    $ azlorad -zmqpubhashtx=tcp://127.0.0.1:218653 \
               -zmqpubhashtx=tcp://192.168.1.2:218653 \
               -zmqpubhashblock="tcp://[::1]:218654" \
               -zmqpubrawtx=ipc:///tmp/azlorad.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in azlora.conf.
...
From the perspective of azlorad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into azlorad directly. Furthermore, no information is
...
using. Azlorad appends an up-counting sequence number to each
