# User-space, Statically Defined Tracing (USDT) for Azlora Core

Azlora Core includes statically defined tracepoints to allow for more
...
**Note**: Azlora Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Azlora Core
...
Multiple tools can list the available tracepoints in a `azlorad` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Azlora Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/azlorad
stap net        outbound_message 0x0000000000107c05 /src/azlorad
stap validation block_connected  0x00000000002fb10c /src/azlorad
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Azlora Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/azlorad | grep NT_STAPSDT -A 4 -B 2
