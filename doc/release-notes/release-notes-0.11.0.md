Azlora Core version 0.11.0 is now available from:

  <https://azlora.org/bin/azlora-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer AZL/kB fee to be
...
For example, add the following to `azlora.conf`:
...
related to the blockchain in the azlora system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Azlora Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Azlora Core will 
...
For an example script see [azlora-submittx](https://github.com/laanwj/azlora-submittx).
...
- #5334 `cf87536` libazloraconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new azlora-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only AzloraAmountField
- #5753 `0f44672` Add azlora logo to about screen
...
- #5809 `46bfbe7` Add azlora-cli man page
...
- #5849 `41259ca` contrib/init/azlorad.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Azlora-Qt.app to Azlora-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for azlora-tx)
