Azlora Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/azlora-core-0.18.0/>
...
`/Applications/Azlora-Qt` (on Mac) or `azlorad`/`azlora-qt` (on
Linux).
...
recommended to use Azlora Core on unsupported systems.

Azlora Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Azlora Core does not yet change appearance when
...
`contrib/init/azlorad.service`, it has been changed to use
`/var/lib/azlorad` as the data directory instead of
`~azlora/.azlora`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/azlorad` will
exist has enough space (check using `df -h /var/lib/azlorad`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/azlora/azlora/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:18653:18653` (this is an extra `:18653` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Azlora Core
...
The systemd init file (`contrib/init/azlorad.service`) has been changed
to use `/var/lib/azlorad` as the data directory instead of
`~azlora/.azlora`. This change makes Azlora Core more consistent with
...
`datadir` option in `/etc/azlora/azlora.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/azlora/azlora.conf`.


Documentation
-------------

- A new short [document](https://github.com/azlora/azlora/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/azlora/azlora/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/azlora/azlora/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/azlora/azlora/blob/master/doc/azlora-conf.md)
  about the `azlora.conf` file describes how to use it to configure
  Azlora Core.

- A new document introduces Azlora Core's BIP174 [Partially-Signed
  Azlora Transactions
  (PSBT)](https://github.com/azlora/azlora/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/azlora/azlora/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Azlora-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Azlora Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/azlora/azlora/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/azlora/azlora/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Azlora Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Azlora Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/azlora/azlora/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/azlora/azlora/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart azlorad with the `-deprecatedrpc=generate`
...
- A new `azlora-wallet` tool is now distributed alongside Azlora
...
This section describes planned changes to Azlora Core that may affect
other Azlora software and services.

- Since version 0.16.0, Azlora Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Azlora Core 0.20 (expected about
  a year after 0.18), Azlora Core will default to native segwit
  ...
  to bech32 addresses, and if the Azlora Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Azlora Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Azlora Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Azlora Core
...
- A sub-project of Azlora Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Azlora Core.  See their [project
  page](https://github.com/azlora-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Azlora Core's own implementation, although entropy
  gathered by Azlora Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Azlora Core a
...
- On macOS, Azlora Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `azlora_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libazlora_server.a` (jnewbery)
...
- #13926 [Tools] azlora-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like azlorad (MarcoFalke)
...
- #14521 qt, docs: Fix `azlora-qt -version` output formatting (hebasto)
...
- #11625 Add AzloraApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libazlora_server` (laanwj)
...
- #15154 Configure: azlora-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/azlora-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Azlora Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libazlora_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in azlora-util-test.py (isghe)
...
- #12775 Integration of property based testing into Azlora Core (Christewart)
...
- #12246 Bugfix: Only run azlora-tx tests when azlora-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_azlora` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_azlora` (MarcoFalke)
...
- #14186 azlora-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_azlora.conf` message on startup if conf file exists (leishman)
...
- #12255 Update azlora.service to conform to init.md (dongcarl)
...
- #14497 Add doc/azlora-conf.md (hebasto)
...
- #15354 Add missing `azlora-wallet` tool manpages (MarcoFalke)
...
- #15765 correct azloraconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support azlora folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/azlora/).
