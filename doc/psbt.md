# PSBT Howto for Azlora Core

Since Azlora Core 0.17, an RPC interface exists for Partially Signed Azlora
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/azlora/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Azlora transactions that are not fully signed
...
#### Multisig with multiple Azlora Core instances
...
Azlora Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `azlora-cli` is used.
...
Later, when *V* AZL has been received on *Amulti*, and Bob and Carol want to
