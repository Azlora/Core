# CJDNS support in Azlora Core

It is possible to run Azlora Core over CJDNS, an encrypted IPv6 network that
...
done outside of Azlora Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Azlora network and individual nodes.
...
before it will work with your Azlora Core node. This is described in steps
...
## Run Azlora Core with CJDNS

Once you are connected to the CJDNS network, the following Azlora Core
...
When enabled, this option tells Azlora Core that it is running in an
...
IPv6 local network. This helps Azlora Core perform better address management:
...
CJDNS support was added to Azlora Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `azlora-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/azlora/azlora/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Azlora Core

There are several ways to see your CJDNS address in Azlora Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `azlora-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `azlora-cli getpeerinfo`).
