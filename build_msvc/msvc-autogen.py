...
def parse_config_into_azl_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Azlora Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/azlora_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/azlora_config.h'), "w", encoding="utf8") as azl_config:
        azl_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Azlora-core msbuild configuration initialiser.')
    ...
    parse_config_into_azl_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/azlora_config.h'), os.path.join(SOURCE_DIR, 'config/azlora-config.h'))
