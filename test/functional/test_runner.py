...
`test/functional/test_framework/test_framework.py:AzloraTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_azlora_cli.py --legacy-wallet',
    'interface_azlora_cli.py --descriptors',
...
def main():
    ...
    enable_azlorad = config["components"].getboolean("ENABLE_AZLORAD")

    if not enable_azlorad:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if azlorad is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "azlorad"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a azlorad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `azlora-cli help` (`rpc_interface.txt`).
