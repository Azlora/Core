...
TMPDIR_PREFIX = "azlora_func_test_"
...
class AzloraTestMetaClass(type):
    """Metaclass for AzloraTestFramework.

    Ensures that any attempt to register a subclass of `AzloraTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'AzloraTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("AzloraTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("AzloraTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class AzloraTestFramework(metaclass=AzloraTestMetaClass):
    """Base class for a azlora test script.

    Individual azlora test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave azlorads and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop azlorads after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use azlora-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_azlorad = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "azlorad" + config["environment"]["EXEEXT"],
        )
        fname_azloracli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "azlora-cli" + config["environment"]["EXEEXT"],
        )
        fname_azlorautil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "azlora-util" + config["environment"]["EXEEXT"],
        )
        self.options.azlorad = os.getenv("AZLORAD", default=fname_azlorad)
        self.options.azloracli = os.getenv("AZLORACLI", default=fname_azloracli)
        self.options.azlorautil = os.getenv("AZLORAUTIL", default=fname_azlorautil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: azlorads were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("AzloraRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'azlorad', self.options.azlorad) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'azlora-cli', self.options.azloracli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a azlorad test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple azlorad test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as azlorad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    azlorad=self.options.azlorad,
                    azlora_cli=self.options.azloracli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in azlora.conf
    ...
    def skip_if_no_azlorad_tracepoints(self):
        """Skip the running test if azlorad has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("azlorad has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_azlorad_zmq(self):
        """Skip the running test if azlorad has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("azlorad has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if azlora-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("azlora-wallet has not been compiled")

    def skip_if_no_azlora_util(self):
        """Skip the running test if azlora-util has not been compiled."""
        if not self.is_azlora_util_compiled():
            raise SkipTest("azlora-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if azlora-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("azlora-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether azlora-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether azlora-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_azlora_util_compiled(self):
        """Checks whether azlora-util was compiled."""
        return self.config["components"].getboolean("ENABLE_AZLORA_UTIL")
